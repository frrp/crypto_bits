(ns bits.crypt.bouncy-castle
  (:require [bits.crypt.api :as api]
            [bits.host :as h :refer [as-big-int as-byte-array]]
            [bits.crypt.sha256 :as sha256])
...
(extend-type bits.crypt.api.RSAPublicKey
  bits.crypt.api/IEncryptor
  (encryptor [key]
    (->CipherProcess (SimpleCipherInit. "BC" "RSA/ECB/PKCS1Padding" key :encrypt)))

  bits.crypt.api/IVerificator
...
(extend-type bits.crypt.api.AESKey
  bits.crypt.api/IEncryptor
  (encryptor [key]
